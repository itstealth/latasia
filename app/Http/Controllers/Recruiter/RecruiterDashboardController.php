    ...
    */
    ...
    public function UpdateLeadOverview(Request $request, Lead $lead)
    {
        ...
        $validated = $request->validate([
            ...
            'trc_country'          => 'nullable|string|max:255',
            'trc_validity'          => 'nullable|string|max:255',
            'trc_expriry_date'          => 'nullable|string|max:255',
            'yellow_card_stamping_date'  => 'nullable|string|max:255',
            'city'  => 'nullable|string|max:255',
            'category_id'  => 'nullable|string|max:255',
            'salary_id'  => 'nullable|string|max:255',
        ]);

        // ✅ Update safely (only validated fields)
    ...
    /**
     * Leads grouped by Employer
     */
    public function leadsByEmployer()
    {
        $recruiter = Auth::guard('recruiter')->user();
        ...
        // ✅ FULL LEADS (NO GROUP BY)
        $leads = Lead::with('employer')
            ->where('recruiter_id', $recruiter->id)
            ->where('lead_disposition', 'interested')
            ->orderBy('employer_id')
            ->get();

        return view('recruiter.employer_lead', compact('leads'));
    }
    ...
    public function sendWhatsapp($id)
    {
        ...
        return response()->json([
            'success' => true,
            'sent_at' => $lead->whatsapp_sent_at->format('d M Y h:i A')
        ]);
    }
